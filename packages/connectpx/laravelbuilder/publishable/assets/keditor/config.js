jQuery(document).ready(function($){
    ...
    var $textarea = $(document).find('textarea[name="body"]');
	var $laravelbuilder = $('<div/>')
		.addClass('laravelbuilder')
        .html( $textarea.val() );

	$textarea.after($laravelbuilder).addClass('laravelbuilder-textarea').hide();

	setTimeout(function(){
		$laravelbuilder.keditor({
			snippetsUrl: laravelbuilder_config.snippet_url,
	        onContentChanged: function (event) {
                console.log('onContentChanged', event);
            },
            onSnippetDropped: function (event, newSection, droppedSnippet) {
                console.log('onSnippetDropped', event, newSection, droppedSnippet);
            },
            onSectionChanged: function (event, changedSection) {
                console.log('onSectionChanged', event, changedSection);
            },
            onBeforeSectionDeleted: function (event, btnRemove, selectedSection) {
                console.log('onBeforeSectionDeleted', event, btnRemove, selectedSection);
            },
            onSectionDuplicated: function (event, originalSection, newSection) {
                console.log('onSectionDuplicated', event, originalSection, newSection);
            },
            onSectionSelected: function (event, selectedSection) {
                console.log('onSectionSelected', event, selectedSection);
            }
	    });
	}, 500);

    $textarea.closest('form').on('submit', function() {
        $textarea.val( $laravelbuilder.keditor('getContent') );
    });
});